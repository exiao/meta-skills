...
The old reflection step (6a/6a.5) was *compressed*: the optimizer saw the input, the final output, the names of failed evals, and a one-line self-diagnosis. It never saw WHERE in the run the skill went wrong. Meta-Harness (Lee et al. 2026, arxiv 2603.28052) shows that this aggressive feedback compression is exactly what makes text optimizers underperform on harness/skill code. Their fix: give the proposer **filesystem access to the source, training scores, and full training execution traces**, and let it browse that evidence agentically before proposing a change. Held-out validation evidence stays outside this browsing context and is used only by the accept/reject gate.
...
**Redact secrets and sensitive data before persisting.** Tool arguments and results are written verbatim, so a skill that uses authenticated tools, private files, or customer data would otherwise leak credentials/PII into the delivered optimization directory. Before writing a trace, run tool arguments and results through a redactor: mask anything matching known secret shapes (API keys, tokens, `Authorization`/`Cookie` headers, passwords, connection strings) and any caller-declared sensitive fields, replacing the value with `[REDACTED:<kind>]`. Prefer an allowlist of fields to persist for known-sensitive tools over a denylist. The divergence-relevant structure (which tool, success/failure, error class) is preserved; only the sensitive *values* are masked.

---

## the trace archive (filesystem the proposer browses)

Every **training** run writes a trace file. The current accepted best's traces live under `cand_best/`; the candidate under test this round writes to `cand_<id>/` and, on a KEEP, becomes the new `cand_best/`. Validation run traces are never exposed to the proposer; if they are captured for debugging, keep them outside this archive:

```
autoresearch-[skill-name]/
└── traces/
    ├── cand_best/                  # the current accepted best
    │   ├── run_input1_r1.md        # one file per (input, run)
    │   ├── run_input1_r2.md
    │   └── ...
    └── cand_<id>/                  # the candidate under test this round
        └── ...
```
...
>
> Investigate before proposing. Read the traces of the failing runs. Find the exact step where each run went wrong, not just that it failed. When the same input passed on one run and failed on another, diff the right run against the wrong run — the difference is the highest-signal evidence you have.
>
...
**Cost control:** reading full traces costs optimizer tokens. Bound it: cap the proposer at reading the failing runs' traces for the current best. If a run's trace exceeds ~2k tokens, the capture step should keep the tool-call/result spine and the turns around the marked divergence, dropping long verbatim tool payloads (store those truncated with a `[...truncated N chars...]` marker). Never drop the divergence window. Do not use validation traces as a cost-control shortcut; they remain sealed from the proposer regardless of size.
