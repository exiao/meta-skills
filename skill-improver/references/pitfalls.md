...
When running autoresearch on skills that exist in both a public repo and a private runtime, skill cross-references may use names that only exist in one context. A runtime may use aliases (e.g. `tweet-ideas`, `article-writer`, `slideshow-creator`) that differ from the public repo's canonical names (e.g. `writer`, `video skills`). Always verify cross-referenced skill names exist in the target context before scoring reference-routing evals.
