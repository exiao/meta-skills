...
`baseline.get(S)` is `None` when the section is new (not in the frozen original), so a section both parents added identically is taken as-is without erroring, and a section only one parent added counts as that parent's evolution. A `delete` edit shows up as `A.S is None` / `B.S is None` against a section that *did* exist in baseline; treat that absence as an evolved value too, so a parent's deliberate deletion is preserved instead of being resurrected from the other parent.
